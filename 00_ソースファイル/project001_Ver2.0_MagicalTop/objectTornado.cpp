...
//	オーバーロードコンストラクタ
//============================================================
CObjectTornado::CObjectTornado(const CObject::LABEL label, const int nPriority) : CObject(label, nPriority)
{
	// メンバ変数をクリア
	m_pVtxBuff = NULL;	// 頂点バッファ
	m_nNumVtx = 0;		// 必要頂点数
...
HRESULT CObjectTornado::Init(void)
{
	...
	m_tornado.cull   = D3DCULL_CCW;	// カリング状況
	m_tornado.bLight = true;		// ライティング状況
...
//	位置の設定処理
//============================================================
void CObjectTornado::SetPosition(const D3DXVECTOR3& rPos)
{
	// 引数の位置を設定
	m_tornado.pos = rPos;
}
...
void CObjectTornado::SetDirectionRotation(const D3DXVECTOR3& rRot)
{
	...
	// 成長向きの正規化
	useful::NormalizeRot(m_tornado.direRot.x);
	useful::NormalizeRot(m_tornado.direRot.y);
	useful::NormalizeRot(m_tornado.direRot.z);
}
...
//	位置取得処理
//============================================================
D3DXVECTOR3 CObjectTornado::GetPosition(void) const
{
	// 位置を返す
