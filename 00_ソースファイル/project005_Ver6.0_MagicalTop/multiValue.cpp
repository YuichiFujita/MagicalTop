...
//	位置の設定処理
//============================================================
void CMultiValue::SetPosition(const D3DXVECTOR3& rPos)
{
	// 引数の位置を設定
	m_pos = rPos;
...
//	向きの設定処理
//============================================================
void CMultiValue::SetRotation(const D3DXVECTOR3& rRot)
{
	for (int nCntValue = 0; nCntValue < m_nDigit; nCntValue++)
	{ // 桁数分繰り返す

		// 引数の向きを全ての数字に設定
		m_apValue[nCntValue]->SetRotation(rRot);
	}
}
...
//	大きさの設定処理
//============================================================
void CMultiValue::SetScaling(const D3DXVECTOR3& rSize)
{
	for (int nCntValue = 0; nCntValue < m_nDigit; nCntValue++)
	{ // 桁数分繰り返す

		// 引数の大きさを全ての数字に設定
		m_apValue[nCntValue]->SetScaling(rSize);
	}
}
...
//	位置取得処理
//============================================================
D3DXVECTOR3 CMultiValue::GetPosition(void) const
{
	// 位置を返す
	return m_pos;
}
...
void CMultiValue::SetNumPosition(void)
{
	...
	{ // 桁数分繰り返す
		...
		// 原点の位置から数字の位置を設定
		m_apValue[nCntValue]->SetPosition(m_pos + (m_space * (float)nCntValue));
	}
}
