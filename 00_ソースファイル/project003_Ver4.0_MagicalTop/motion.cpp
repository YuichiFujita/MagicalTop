...
void CMotion::Set(const int nType)
{
	...
	{ // モデルのパーツ数分繰り返す
		...
		// 初期位置と初期向きを設定
		m_ppModel[nCntKey]->SetPosition(m_info.aMotionInfo[nType].aKeyInfo[m_info.nPose].aKey[nCntKey].pos);
		m_ppModel[nCntKey]->SetRotation(m_info.aMotionInfo[nType].aKeyInfo[m_info.nPose].aKey[nCntKey].rot);
	}
}
...
//	モーション情報の設定処理
//============================================================
void CMotion::SetInfo(const MotionInfo info)
{
	// 引数のモーション情報を設定
