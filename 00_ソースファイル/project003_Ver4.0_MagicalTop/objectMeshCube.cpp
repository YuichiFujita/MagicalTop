...
//	オーバーロードコンストラクタ
//============================================================
CObjectMeshCube::CObjectMeshCube(const CObject::LABEL label, const int nPriority) : CObject(label, nPriority)
{
	// メンバ変数をクリア
	m_pVtxBuff = NULL;	// 頂点バッファ
	m_pIdxBuff = NULL;	// インデックスバッファ
...
HRESULT CObjectMeshCube::Init(void)
{
	...
	m_meshCube.bordState  = BORDERSTATE_OFF;	// 縁取り使用状態
...
void CObjectMeshCube::Draw(void)
{
	...
	// 縁取りの描画
	if (m_meshCube.bordState == BORDERSTATE_ON)
	{ // 縁取りがONの場合
...
//	テクスチャ割当処理
//============================================================
void CObjectMeshCube::BindTexture(const FACETEX textureID)
{
	// テクスチャインデックスを代入
...
//	位置の設定処理
//============================================================
void CObjectMeshCube::SetPosition(const D3DXVECTOR3& rPos)
{
	// 引数の位置を設定
	m_meshCube.pos = rPos;
}
...
//	向きの設定処理
//============================================================
void CObjectMeshCube::SetRotation(const D3DXVECTOR3& rRot)
{
	// 引数の向きを設定
	m_meshCube.rot = rRot;

	// 向きの正規化
	useful::NormalizeRot(m_meshCube.rot.z);
}

//============================================================
//	大きさの設定処理
//============================================================
void CObjectMeshCube::SetScaling(const D3DXVECTOR3& rSize)
{
	// 引数の大きさを設定
...
//	縁取りの状態の設定処理
//============================================================
HRESULT CObjectMeshCube::SetBorderState(const BORDERSTATE bordState)
{
	// ポインタを宣言
	LPDIRECT3DDEVICE9 pDevice = CManager::GetRenderer()->GetDevice();	// デバイスのポインタ
...
//	テクスチャの状態の設定処理
//============================================================
void CObjectMeshCube::SetTextureState(const TEXSTATE texState)
{
	// 引数のテクスチャの状態を設定
...
{
	// 引数のテクスチャの分割数Xを設定
	m_meshCube.aTexPart[0] = rTexPart;
...
{
	// 引数のテクスチャの分割数Yを設定
	m_meshCube.aTexPart[1] = rTexPart;
...
{
	// 引数のテクスチャの分割数Zを設定
	m_meshCube.aTexPart[2] = rTexPart;
...
//	位置取得処理
//============================================================
D3DXVECTOR3 CObjectMeshCube::GetPosition(void) const
{
	// 位置を返す
...
//	向き取得処理
//============================================================
D3DXVECTOR3 CObjectMeshCube::GetRotation(void) const
{
	// 向きを返す
...
//	大きさ取得処理
//============================================================
D3DXVECTOR3 CObjectMeshCube::GetScaling(void) const
{
	// 大きさを返す
...
{
	// 縁取り色を返す
	return m_meshCube.aCol[1];
}

//============================================================
//	縁取り状態取得処理
//============================================================
CObjectMeshCube::BORDERSTATE CObjectMeshCube::GetBorderState(void) const
{
	// 縁取りの状態を返す
...
//	テクスチャ状態取得処理
//============================================================
CObjectMeshCube::TEXSTATE CObjectMeshCube::GetTextureState(void) const
{
	// テクスチャの状態を返す
...
void CObjectMeshCube::SetVtx(void)
{
	...
	D3DXVECTOR2 texPart = D3DXVECTOR2(0.0f, 0.0f);	// テクスチャ分割数の計算用
	...
	{ // 使用中の場合
		...
		{ // 縁取りがONの場合二回繰り返す
			...
			{ // 必要頂点・インデックス数分繰り返す
				...
				// インデックスの設定
				pIdx[0] = aIdxMeshCube[nCntSet] + nNumVtx;
